...
from model_vc_world_neo import GeneratorNeo
from dataset_world_v2 import AudiobookDataset
from dataset_world_v2 import train_collate_world, test_collate_world
...
def train(args, model, device, train_loader, optimizer, epoch, sigma=1.0):
    ...
    for batch_idx, (s, a, e) in enumerate(train_loader):
        s = s.to(device)
        a = a.to(device)
        e = e.to(device)
        ...
        so, sop, soc, ao, aop, aoc = model(s, a, e, e)
        socr, aocr = model(s, a, e, None)

        L_recon_s   = ((sop - s) ** 2).sum(dim=(1,2)).mean()
        L_recon_a   = ((aop - a) ** 2).sum(dim=(1,2)).mean()
        L_recon0_s  = ((so - s) ** 2).sum(dim=(1,2)).mean()
        L_recon0_a  = ((ao - a) ** 2).sum(dim=(1,2)).mean()
        L_content_s = torch.abs(soc - socr).sum(dim=1).mean()
        L_content_a = torch.abs(aoc - aocr).sum(dim=1).mean()
        
        loss = ((L_recon_s + L_recon_a) / 2) + ((L_recon0_s + L_recon0_a) / 2) + ((L_content_s + L_content_a) / 2)
        ...
        if batch_idx % 10 == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                epoch, batch_idx * len(s), len(train_loader.dataset),
        ...
        for batch_idx, (s, a, e) in enumerate(train_loader):
            ...
            if batch_idx % 100 == 0:
                print('Val Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                    epoch, batch_idx * len(s), len(test_loader.dataset),
...
if __name__ == '__main__':
    ...
    model = GeneratorNeo(hp.dim_neck, hp.dim_emb, hp.dim_pre, hp.freq).to(device)
