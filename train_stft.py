...
from dataset import stft_train_collate
from dataset import stft_test_collate
...
def train(args, model, device, train_loader, optimizer, epoch, sigma=1.0):
    ...
    for batch_idx, (m, e) in enumerate(train_loader):
        ...
        L_recon = ((mel_outputs_postnet - m) ** 2).sum(dim=(1,2,3)).mean()
        L_recon0 = ((mel_outputs - m) ** 2).sum(dim=(1,2,3)).mean()
        L_content = torch.abs(codes - codes_rec).sum(dim=(1,2)).mean()
...
if __name__ == '__main__':
    ...
    model = GeneratorStft(hp.dim_neck, hp.dim_emb, hp.dim_pre, hp.freq).to(device)
