...
def complex_matmul(A, B):
    ...
    return outp_real.type(torch.cfloat) + 1j * outp_imag.type(torch.cfloat)
...
def complex_avg_pool2d(input, *args, **kwargs):
    ...
    return absolute_value_real.type(torch.cfloat)+1j*absolute_value_imag.type(torch.cfloat)
...
def complex_normalize(input):
    ...
    return real_norm.type(torch.cfloat) + 1j*imag_norm.type(torch.cfloat)

def complex_relu(input):
    return relu(input.real).type(torch.cfloat)+1j*relu(input.imag).type(torch.cfloat)
...
def complex_upsample2(input, size=None, scale_factor=None, mode='nearest',
    ...
    return outp_abs \
           * (torch.cos(angle).type(torch.cfloat)+1j*torch.sin(angle).type(torch.cfloat))
...
def complex_max_pool2d(input,kernel_size, stride=None, padding=0,
    ...
    absolute_value = absolute_value.type(torch.cfloat)
