...
from torch.nn import ConvTranspose2d, LSTM, Conv1d
from complex.ComplexFunctions import complex_relu, complex_max_pool2d, complex_avg_pool2d
from complex.ComplexFunctions import complex_dropout, complex_dropout2d
...
class ComplexBatchNorm2d(_ComplexBatchNorm):
    ...
    def forward(self, input):
        ...
        if self.training or (not self.training and not self.track_running_stats):
            ...
            mean_r = input.real.mean([0, 2, 3]).type(torch.cfloat)
            mean_i = input.imag.mean([0, 2, 3]).type(torch.cfloat)
        ...
        input = (Rrr[None,:,None,None]*input.real+Rri[None,:,None,None]*input.imag).type(torch.cfloat) \
                + 1j*(Rii[None,:,None,None]*input.imag+Rri[None,:,None,None]*input.real).type(torch.cfloat)

        if self.affine:
            input = (self.weight[None,:,0,None,None]*input.real+self.weight[None,:,2,None,None]*input.imag+\
                    self.bias[None,:,0,None,None]).type(torch.cfloat) \
                    +1j*(self.weight[None,:,2,None,None]*input.real+self.weight[None,:,1,None,None]*input.imag+\
                    self.bias[None,:,1,None,None]).type(torch.cfloat)

        return input

class NavieComplexLSTM(Module):
    def __init__(self, input_size, hidden_size, num_layers=1, bidirectional=False, batch_first=False):
        ...
        self.real_lstm = LSTM(self.input_dim, self.rnn_units, num_layers=num_layers, bidirectional=bidirectional, batch_first=batch_first)
        self.imag_lstm = LSTM(self.input_dim, self.rnn_units, num_layers=num_layers, bidirectional=bidirectional, batch_first=batch_first)
        if bidirectional:
            bidirectional=2
        else:
            bidirectional=1
    ...
    def forward(self, inputs):
        ...
        return real_out.type(torch.cfloat) + 1j*imag_out.type(torch.cfloat)
